...
function repairTableHeader(headers){
    ...
    /*
     * Header beginning with Factor.
     */

    if(
        /^Factor/i.test(header)
    ){
        const remainder =
            header
                .replace(
        ...
        for(const firstOption of optionPatterns){
            ...
            if(
                ...
                for(const secondOption of optionPatterns){

                    if(
                        afterFirst
                            .toLowerCase()
                            .startsWith(
                                secondOption.toLowerCase()
                            )
                    ){
        ...
        /*
         * Generic fallback if Factor exists
         * but the option names are unusual.
         */
...
if(text.length > 1200){
    showToast(
        "Message is limited to 1200 characters."
    );
